...
let MagicEventJoint = cc.Class({
    ...
    createFunc(exp) {
        //注入两个变量：sender\event
        let func = new Function('sender', 'event', `${exp}`);
...
if (CC_EDITOR) {
    cc.Class.Attr.setClassAttr(MagicEventJoint, 'senderPath', 'visible', function() {
        return !this.advanced;
    });

    cc.Class.Attr.setClassAttr(MagicEventJoint, 'handlePath', 'visible', function() {
        return !this.advanced;
    });

    cc.Class.Attr.setClassAttr(MagicEventJoint, 'expression', 'visible', function() {
        return this.advanced;
    });
}

module.exports = MagicEventJoint;